...
def compare_methods(master_p, declared_perms_list):
                ...
                for k,v in master_p[i].items():         # for all of the key-value pairs in the dictionaries we're looking for
                    ...
                    if match_dict and match_dict not in list:   # check to see if match_dict is NOT blank, i.e. { }, { } or already present
                        ...
                        if call_perm not in declared_perms_list:
                            ...
                            if call_perm not in undeclared_perms:
                                undeclared_perms[call_perm] = []
                                undeclared_perms[call_perm].append(match_dict)
                            else:
                                undeclared_perms[call_perm].append(call_perm)

    print "ending time: ", datetime.datetime.now()

    return list, undeclared_perms
